	...
	<div style="text-align: center;" class="container">
		...
		<div class="contents text-center">
			...
			<form method="post" action="${hContext }/member/forgotPassword.do" class="" name="loginForm" id="loginForm">
				...
				<input style="width: 400px; margin: 5px;" class="btn btn-danger btn-lg" type="button" value="비밀번호 찾기" id="forgotPassword"><br>
				<input style="width: 400px; margin: 5px;" class="btn btn-default btn-lg" type="button" value="뒤로가기" id="doBackBtn"><br><br>
				<a href="/board/member/moveToForgotId.do">혹시 아이디가 기억나지 않나요?</a>
			</form>
		</div>	<!-- end contents -->
...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.2/jquery.min.js"></script>
	...
	// 비밀번호 재설정 버튼
		$("#forgotPassword").on("click", function(){
			...
			var id = $("#inputMemberId").val().trim();
			console.log("email : "+email);
			if(id == ""){
				document.getElementById('inputMemberId').focus();
				alert("아이디를 입력하세요.");
				return;
			}
			if(email == ""){
			...
			$.ajax({
				type : 'POST',
				url : '${hContext}/member/forgotPassword.do',
				dataType : "html",
				async : true,
				data : {
				...
				success : function(data) {
					...
					alert("비밀번호는 "+parseData.password+" 입니다.");
					window.location.href = "${hContext}/member/loginView.do";
				},
				error : function() {
